jQuery(document).ready(function(e){
    var $ = jQuery;    
    var baseSelector = '#region-sidebar_slideshow #view_custom_lasooo_navigation';
    var baseTargetSelector = "#region-sidebar_slideshow"; //['id'!='view_custom_lasooo_navigation']
    var activedBlock = [];    
    ...
    $(baseSelector+" .item").each(function(e){
        $(this).bind('click', function(e){
            ...
            if (ids[1].length>0 && $(baseTargetSelector+" "+ ids[1]).length>0 && 'none'==$(baseTargetSelector+" "+ ids[1]).css('display')) {
                $(baseTargetSelector+">div[id!='view_custom_lasooo_navigation']").slideUp();
                $(baseTargetSelector+" "+ ids[1] + " > div").show('slow');
                $(baseTargetSelector+" "+ ids[1]).slideDown();
                //addCloseButton(baseTargetSelector+" "+ ids[1]);
                enlightButton(this);
            }else{
                $(baseTargetSelector+" "+ ids[1] + " > div").hide('normal');
    ...
    $(baseTargetSelector+">div[id!='view_custom_lasooo_navigation']").each(function(e){
