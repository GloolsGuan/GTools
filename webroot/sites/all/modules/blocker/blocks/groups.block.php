 ...
 * */
function blocker_block_groups_info(){
	return array(
        'info'  => t('Reading group icons'),
        /*
...
function blocker_block_groups_view($delta){
	
	return array(
        'subject' => t('Groups'),
        'content' => array(blocker_block_groups_getData(), array('#children'=>'<div class="clearfix"></div>'))
    );
}


function blocker_block_groups_getData(){
    $rQ = db_query('select u.* from {users} as u left join ({role} as r, {users_roles} as rs)on(u.uid=rs.uid and r.rid=rs.rid) where r.name=\'reading club\' and u.status=1 order by created desc;');
    
    if ($rQ->rowCount()>0) {
        $clubs = $rQ->fetchAllAssoc('uid');
        return member_buildAvatars($clubs, 'group');
    }
